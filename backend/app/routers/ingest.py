...
from app.services.ingest import run_all
...
def run_kaggle(req: KaggleRequest, background_tasks: BackgroundTasks):
    ...
    def _task():
        ...
        _set_running("kaggle")
        try:
            n = import_from_kaggle_csv(db, req.file_path, limit=req.limit)
...
def run_api_football(req: ApiFootballRequest, background_tasks: BackgroundTasks):
    ...
    def _task():
        ...
        try:
            n = _run_async_in_thread(fetch_from_api_football(db, req.league_id, req.season))
    ...
    def _task():
        ...
        try:
            result = _run_async_in_thread(
                fetch_from_statsbomb(db, req.competition_id, req.season_id, req.max_matches)
            )
...
def run_football_data(req: FootballDataRequest, background_tasks: BackgroundTasks):
    ...
    def _task():
        ...
        try:
            result = _run_async_in_thread(
                sync_player_clubs(db, req.competition_code, req.season)
            )
...
def run_all_sources(req: RunAllRequest, background_tasks: BackgroundTasks):
    ...
    def _task():
        _set_running("all")
        try:
            result = _run_async_in_thread(run_all(
                kaggle_file=req.kaggle_file,
                kaggle_limit=req.kaggle_limit,
                api_league=req.api_league,
                season=req.season,
                statsbomb_comp=req.statsbomb_comp,
                statsbomb_season_id=req.statsbomb_season_id,
                statsbomb_max_matches=req.statsbomb_max_matches,
                fbref_league=req.fbref_league,
                fbref_season=req.fbref_season,
                football_data_comp=req.football_data_comp,
            ))
            _set_done("all", result)
        except Exception as e:
            _set_error("all", str(e))
