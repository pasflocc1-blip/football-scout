"""
services/search.py — v2.2
-----------------------------------------------------------------
FIX: rimossi riferimenti a M.xg_per90, M.xa_per90, M.npxg_per90
     che non esistono più su ScoutingPlayer dopo la ristrutturazione.
     Le keyword basate su xG ora usano i percentili/score già calcolati.

Il search semantico usa solo colonne di ScoutingPlayer:
- score oggettivi (finishing_score, creativity_score, ecc.)
- percentili (*_pct)
- posizione, età, piede, nazionalità
"""
...
SEMANTIC_MAP: dict = {
    ...
    # ── xG / xA — ora basati sugli score calcolati (non direttamente su xg_per90) ──
    "alto xg": lambda: _pct_or_score("finishing_pct", "finishing_score", 70, 60),
    "alto xa": lambda: _pct_or_score("creativity_pct", "creativity_score", 70, 60),
    "npxg alto": lambda: _pct_or_score("finishing_pct", "finishing_score", 75, 65),
    "xg chain": lambda: _pct_or_score("buildup_pct", "buildup_obj_score", 65, 55),
...
def _has_data_expr():
    return case(
        (M.finishing_score.isnot(None) | M.creativity_score.isnot(None), 0),
        else_=1,
    )
